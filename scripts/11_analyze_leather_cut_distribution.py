...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze real and synthetic leather cut distributions.")
    parser.add_argument("--data-root", type=Path, default=None, help="Path to the MVTec AD dataset root. Defaults to DATA_ROOT.")
    parser.add_argument("--category", default="leather")
    parser.add_argument("--old-traditional-summary", type=Path, required=True)
    parser.add_argument("--old-diffusion-summary", type=Path, required=True)
    parser.add_argument("--new-traditional-summary", type=Path, required=True)
    parser.add_argument("--new-diffusion-summary", type=Path, required=True)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs") / "stage11_leather_precision_cut_fix" / "analysis",
    )
...
def load_real_rows(data_root: Path, category: str) -> list[dict[str, object]]:
    image_dir = data_root / category / "test" / "cut"
    mask_dir = data_root / category / "ground_truth" / "cut"
...
def main() -> None:
    ...
    write_csv(summary_rows, args.output_dir / "leather_cut_distribution_summary.csv")
    write_csv(rows, args.output_dir / "leather_cut_distribution_samples.csv")

    print("Leather cut distribution analysis finished.")
