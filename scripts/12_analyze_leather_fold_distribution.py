...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze real and synthetic leather fold distributions.")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs") / "stage12_leather_fold_fix" / "analysis",
    )
...
def load_real_rows(data_root: Path, category: str) -> list[dict[str, object]]:
    image_dir = data_root / category / "test" / DEFECT_TYPE
    mask_dir = data_root / category / "ground_truth" / DEFECT_TYPE
...
def main() -> None:
    ...
    write_csv(summary_rows, args.output_dir / "leather_fold_distribution_summary.csv")
    write_csv(rows, args.output_dir / "leather_fold_distribution_samples.csv")

    print("Leather fold distribution analysis finished.")
